...
def extract_company_name(text: str) -> Union[str, None]:
    ...
    for pattern in patterns:
        if match := re.search(pattern, text):
            ...
            company = re.sub(
                r'^(?:M/s\.|M/s|Messrs\.|cr\s+|projects\.|Projects\.|project\.|Project\.)\s*', '', company)
            company = re.sub(
                r'\s+(?:Private|Pvt|Public|Company)\s+(?:Limited|Ltd)$', ' Limited', company)
            company = re.sub(
                r'\s+(?:Private|Pvt|Public|Company)$', '', company)
            ...
            for retry in range(max_retries):
                try:
                    ...
                    if search_results and search_results.results:
                        for result in search_results.results:
                            ...
                            content = exa.get_contents(
                                urls=[result.url],
                                text={
                                    "max_characters": Config.EXA_SETTINGS['search_parameters']['max_characters']}
                            )
                    ...
                    if retry == max_retries - 1:  # Last retry
                        logger.error(
                            f"Error in Exa search for query '{query}' after {max_retries} retries: {str(e)}")
        ...
        for project in state['projects']:
            try:
                ...
                company_name = project.get(
                    'company') or extract_company_name(text)
                ...
                if any(term in all_text for term in steel_terms):
                    if any(term in all_text for term in ['jsw', 'jindal', 'neosteel', 'trusteel']):
                        jsw_projects.append(project)
                        logger.info(
                            f"Filtered JSW-related steel project: {title}")
                        continue
                ...
                if 'jsw' in all_text and any(term in all_text for term in [
                    ...
                    jsw_projects.append(project)
                    logger.info(
                        f"Filtered JSW supplier/partner project: {title}")
                    continue
                ...
                project.update({
                    ...
                    'description': project.get('description', '')[:2000]
                })
...
def generate_catchy_headline(project: dict) -> str:
    ...
    try:
        ...
        * MEIL to construct 200-km irrigation canal in Andhra
        ...
        headline = re.sub(r'(?i)\b(ltd|limited|corp|corporation)\b',
                          '', headline).strip()  # Remove company suffixes

        # Remove any company headers or extra lines
        # Take only the last line if multiple lines
            ...
            for retry in range(max_retries):
                try:
                    ...
                    if any(term in all_text for term in jsw_product_terms):
                        logger.info(
                            f"Filtered JSW-related project during enrichment: {enriched.get('company')} - {enriched.get('title')}")
                        break
                    ...
                    if enriched.get('company'):
                        logger.info(
                            f"Finding procurement contacts for {enriched['company']}")
                        enriched = contact_finder.enrich_project_contacts(
                            enriched)
                    ...
                    if retry == max_retries - 1:  # Last retry
                        logger.error(
                            f"Error enriching project {project.get('title')} after {max_retries} retries: {str(e)}")
                    time.sleep(1)  # Wait before retry
                    continue
...
def prioritize_projects(state: WorkflowState) -> WorkflowState:
    ...
    try:
        logger.info(
            f"Prioritizing {len(state['enriched_projects'])} projects...")
        ...
        for project in state['enriched_projects']:
            try:
                ...
                steel_req = project.get(
                    'steel_requirements', {}).get('total', 0)
                if steel_req >= 10000:
        ...
        if len(prioritized_projects) < min_projects:
            logger.warning(
                f"Only {len(prioritized_projects)} projects available after prioritization")
...
def send_notifications(state: WorkflowState) -> WorkflowState:
    ...
    try:
        ...
        email_success = email_handler.send_project_opportunities(
            state['prioritized_projects'])
        if not email_success:
            logger.error("Failed to send email notifications")

        # Send WhatsApp notifications
        whatsapp_success = whatsapp_handler.send_project_opportunities(
            state['prioritized_projects'])
...
def create_workflow(data) -> Graph:
...
def run_workflow(data) -> dict:
    ...
    try:
        ...
        logger.info(
            f"Projects filtered: {len(final_state.get('filtered_projects', []))}")
        logger.info(
            f"Projects enriched: {len(final_state.get('enriched_projects', []))}")
        logger.info(
            f"Projects prioritized: {len(final_state.get('prioritized_projects', []))}")
